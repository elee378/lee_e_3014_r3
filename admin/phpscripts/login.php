	...
	function logIn($username, $password, $ip) {
		...
		if(mysqli_num_rows($user_set)) {
			...
			if(mysqli_query($link, $loginString)) {
				...
				$updateTotalLogins = "UPDATE tbl_user SET user_totalLogins = user_totalLogins + 1 WHERE user_id = '$id'";
				$updateTotalLoginsQuery = mysqli_query($link, $updateTotalLogins);
				$loginsString = "SELECT user_totalLogins FROM tbl_user WHERE user_id ='$id' AND user_totalLogins = '1'";
				$loginsQuery = mysqli_query($link, $loginsString);
				$firstLogin = mysqli_num_rows($loginsQuery);
